...
static void test_pedersen(void) {
    ...
    totalv = 0;
    for (i = 0; i < inputs; i++) {
        values[i] = secp256k1_testrands64(0, INT64_MAX - totalv);
        totalv += values[i];
    }
    for (i = 0; i < outputs - 1; i++) {
        values[i + inputs] = secp256k1_testrands64(0, totalv);
...
void test_multiple_generators(void) {
    ...
    for (i = 0; i < n_outputs; i++) {
        value[n_inputs + i] = secp256k1_testrands64(0, INT64_MAX - total_value);
        total_value += value[n_inputs + i];
    }
    for (i = 0; i < n_inputs - 1; i++) {
        value[i] = secp256k1_testrands64(0, total_value);
        total_value -= value[i];
    }
    value[i] = total_value;
